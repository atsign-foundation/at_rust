...
/// Construct an AES key from a decoded key.
pub fn construct_aes_key(decoded_key: &[u8]) -> Box<dyn SynchronousStreamCipher> {
    // The IV is all zeroes. This is how it is done at AtSign.
    // NOTE: In the future I might need to pass in IV as an argument.
...
pub fn decrypt_key(cypher: &mut Box<dyn SynchronousStreamCipher>, encrypted_key: &[u8]) -> String {
    ...
    // TODO: Optimisation: Initialise length to encrypted_key.len()
...
/// Construct an RSA key from a decoded key.
pub fn construct_rsa_key(decoded_key: &[u8]) -> RsaPrivateKey {
    let rsa_key =
...
mod test {
    ...
    fn construct_aes_key_test() {
        ...
        // Act
        let mut cipher = construct_aes_key(&decoded_key);
    ...
    fn decrypt_private_key_test() {
        ...
        let mut cipher = construct_aes_key(&decoded_key);
        // Act
        let output = decrypt_key(
            &mut cipher,
            &decode_base64_text(PKAM_KEY_ENCRYPTED_AND_ENCODED),
        );
        // Assert
        assert_eq!(output, PKAM_KEY_DECRYPTED_AND_ENCODED);
    }
    ...
    fn construct_rsa_key_test() {
        ...
        let output_encoded = decode_base64_text(&output);
        println!("output_encoded: {:?}", output_encoded);
        // Act
        let _ = construct_rsa_key(&output_encoded);
